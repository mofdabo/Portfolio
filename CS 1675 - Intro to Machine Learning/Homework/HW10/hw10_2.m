...
for i = 1:10
    for j = 1:20
       train_error = Bag_classifier(train_x, train_y, train_x, sprintf('[@DT_base_full, %d, []]', i));
       test_error = Bag_classifier(train_x, train_y, test_x, sprintf('[@DT_base_full, %d, []]', i));
       average_error = find_average(train_y, train_error, test_y, test_error);
       bagging_error(i,:) = bagging_error(i,:) + average_error;

       train_error = Boost_classifier(train_x, train_y, train_x, sprintf('[@DT_base_full, %d, []]', i));
       test_error = Boost_classifier(train_x, train_y, test_x, sprintf('[@DT_base_full, %d, []]', i));
       average_error = find_average(train_y, train_error, test_y, test_error);
